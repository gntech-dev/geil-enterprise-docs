...
- [Active Directory Network Requirements](active-directory-network-requirements.md) implemented on `HQ-FW01`.
- [DNS and DHCP Implementation](../microsoft-core/dns-dhcp-implementation.md) completed for VLAN 30.
- [Group Policy Baseline](../microsoft-core/group-policy-baseline.md) created and linked to the Workstations OU.
...
STOP if DHCP works but these tests fail. DHCP relay is not sufficient for domain join. Return to [Active Directory Network Requirements](active-directory-network-requirements.md) and verify address lists and service rules before continuing.
...
|---|---|---|
| DHCP works but DNS times out | AD service firewall rules missing or below default deny | Apply [Active Directory Network Requirements](active-directory-network-requirements.md). |
