...
- Execute privileged validation from `HQ-MGMT01`, the Windows 11 Enterprise management workstation / initial PAW, where possible. Do not use Windows Server as a daily admin workstation.
...
- Audit focus: Validate bridges, MikroTik CHR, VM placement, and evidence before acceptance.
...
- What the `Phase 1 validation` workstream changes.
...
- ✓ Completed the `Phase 1 validation` workstream.
...
- `Phase 1 validation` is complete and validated.
