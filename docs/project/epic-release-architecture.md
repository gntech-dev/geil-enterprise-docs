...
| 6 | Windows Event Forwarding | Pilot In Progress | Centralizes events using dedicated collector `HQ-WEC01` once WinRM and firewall dependencies are in place. |
...
| Windows Event Forwarding | Centralize important Windows security and operational events using dedicated collector `HQ-WEC01`. | Pilot In Progress | Enterprise WinRM Management; Windows Firewall Baseline; Microsoft Defender Baseline; Windows LAPS Baseline. | [Windows Event Forwarding and Collector Baseline](../microsoft-core/windows-monitoring/windows-event-forwarding-baseline.md). | WEC collector design; source-initiated subscriptions; `HQ-W11-001` and `HQ-MGMT01` pilot sources; client-to-collector TCP `5985`; Forwarded Events validation; future SIEM integration. |
...
| WIN.R01 | E05 | Secure Remote Administration Foundation | In progress: Management Workstation, RDP validation, WinRM, MikroTik Windows management firewall policy, Network and AD services matrix |
| WIN.R02 | E06 | Host Firewall and Local Admin Protection | In progress: Windows Firewall Baseline and Windows LAPS |
| WIN.R03 | E06 | Endpoint Protection and Monitoring | Done: Microsoft Defender Baseline; pilot in progress: Windows Event Forwarding |
| WIN.R04 | E03 | Privileged Access Maturity | Planned: Tier 0/1/2 model, PAW refinement, JEA, role-based administration, future certificate-based WinRM HTTPS |
